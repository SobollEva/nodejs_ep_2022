...
export function getAutoSuggestUsers(loginSubstring: string, limit: number, userList: User[]): User[] {
    ...
    const newStore = userList.length > 1
        ? userList.sort(sortListByLogin)
...
export function getUserIndex(userStore: User[], userId: string): number| null {
    const userIndex = userStore.findIndex((item: User) => item.id === userId);

    return Boolean(userIndex + 1) ? userIndex: null;
}
